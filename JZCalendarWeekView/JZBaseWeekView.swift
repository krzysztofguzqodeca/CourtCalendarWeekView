...
open class JZBaseWeekView: UIView {
    ...
    */
    ...
    open func setup() {
        ...
        collectionView.backgroundColor = JZWeekViewColors.rowsBackgroundColor
    ...
    open func registerViewClasses() {
        // supplementary
        self.collectionView.registerSupplimentaryViews([JZColumnHeader.self, JZCornerHeader.self, JZRowHeader.self])
    ...
    private func getSectionWidth() -> CGFloat {
        var sectionWidth = flowLayout.sectionWidth
...
//    }
    ...
    open func updateWeekView(to date: Date) {
        self.initDate = date.startOfDay.add(component: .day, value: 0)
        DispatchQueue.main.async { [unowned self] in
            self.layoutSubviews()
            self.forceReload()
        }
    ...
    */
    ...
    open func getSectionForPointX(_ xCollectionView: CGFloat) -> Int {
        // RowHeader(horizontal UICollectionReusableView) should be considered in gesture point
        // Margin area for point X can also get actual date, because it is always the middle view unlike point Y
        let adjustedX = xCollectionView - flowLayout.rowHeaderWidth - flowLayout.contentsMargin.left
        let section = Int(adjustedX / flowLayout.sectionWidth)
    ...
    open func getDateForPointY(_ yCollectionView: CGFloat) -> (hour: Int, minute: Int) {
        ...
        var adjustedY = yCollectionView - flowLayout.columnHeaderHeight - flowLayout.contentsMargin.top
        let minY: CGFloat = 0
        // contentSize includes all reusableView, margin and scrollable area
        let maxY = collectionView.contentSize.height - flowLayout.contentsMargin.top - flowLayout.contentsMargin.bottom - flowLayout.columnHeaderHeight
...
extension JZBaseWeekView: UICollectionViewDataSource {
    ...
    open func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if let events = allEventsBySectionInt[section] {
            return events.count
        } else {
            return 0
        }
...
extension JZBaseWeekView: WeekViewFlowLayoutDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout: JZWeekViewFlowLayout, courtForSection section: Int) -> Date {
        return getDateForSection(section)
    }
    ...
    public func collectionView(_ collectionView: UICollectionView, layout: JZWeekViewFlowLayout, startTimeForItemAtIndexPath indexPath: IndexPath) -> Date {
        ...
        if let events = allEventsBySectionInt[indexPath.section] {
